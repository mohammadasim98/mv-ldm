    ...
    def sample(
        ...
        if self.cfg.num_target_views > 0:
            ...
            if self.stage == "test":
                ...
                indices = torch.multinomial(torch.ones_like(index_target, dtype=torch.float), num_samples=self.cfg.num_target_views, replacement=False)
                index_target = index_target[indices.long()]
        else:
            index_target = None
