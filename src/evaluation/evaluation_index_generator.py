    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.cfg.all:
            ...
            views.append(IndexEntry(
                context=tuple(torch.sort(batch["target"]["index"][0][0:1]).values.tolist()),
                target=tuple(torch.sort(batch["target"]["index"][0][1:]).values.tolist())
            ))
            
        else:
            context_indices = torch.randperm(v, generator=self.generator)
            for context_index in context_indices:
                xy, _ = sample_image_grid((h, w), device, dtype)
