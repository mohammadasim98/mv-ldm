...
@dataclass
class MultiViewFeatureTransformerCfg:
    name: Literal["multi_view_feature", "multiview_standard"]
    num_layers: int=1,
    d_model: int=128,
    nhead: int=1,
    attention_type: str="swin",
    ffn_dim_expansion: int=4,
    add_per_view_attn: bool=False,
...
class MultiViewFeatureTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # multi_view_features: list of [B, V, C, H, W]
        b, v, c, h, w = multi_view_features.shape
        assert self.d_model == c, f"d_model {self.d_model} is not equal to feature channels {c}"
        ...
        if self.attention_type == "swin" and attn_num_splits > 1:
            ...
            shifted_window_attn_mask = generate_shift_window_attn_mask(
                ...
                shift_size_h=window_size_h // 2,
                shift_size_w=window_size_w // 2,
                device=multi_view_features.device,
            )  # [K*K, H/K*W/K, H/K*W/K]
        else:
            shifted_window_attn_mask = None

        # [N*B, C, H, W], [N*B, N-1, C, H, W]
        multi_view_features = [multi_view_features[:, i] for i in range(multi_view_features.shape[1])]
