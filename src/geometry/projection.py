...
def get_world_rays(
    coordinates: Float[Tensor, "... dim"],
    extrinsics: Float[Tensor, "... dim+2 dim+2"],
    intrinsics: Float[Tensor, "... dim+1 dim+1"],
) -> tuple[
    Float[Tensor, "... dim+1"],  # origins
    Float[Tensor, "... dim+1"],  # directions
]:
...
def sample_image_grid(
    ...
    coordinates = [((idx + 0.5) / length).to(dtype=dtype) for idx, length in zip(indices, shape)]
...
def sample_training_rays(
    ...
    xy, _ = sample_image_grid(tuple(grid_shape), device, dtype)
